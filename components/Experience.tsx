    ...
    <div className="w-full max-w-5xl mx-auto py-6 space-y-8">
      {RESUME_DATA.experience.map((job, index) => (
        <motion.div
          ...
          className="group relative bg-white dark:bg-[#181818]/80 backdrop-blur-sm p-6 sm:p-8 rounded-2xl border border-slate-200 dark:border-[#2a2a2a] hover:border-accent/60 transition-all duration-300 shadow-md dark:shadow-xl hover:shadow-xl hover:shadow-accent/10"
        >
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-6 border-b border-slate-200 dark:border-[#2a2a2a]/60">
            ...
            <div className="flex flex-wrap items-center gap-3 font-mono text-xs">
              <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-slate-100 dark:bg-[#101010] border border-slate-200 dark:border-[#2a2a2a] text-slate-700 dark:text-slate-300">
                <Calendar size={13} className="text-accent" />
                {job.period}
              </span>
              <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-slate-100 dark:bg-[#101010] border border-slate-200 dark:border-[#2a2a2a] text-slate-600 dark:text-slate-400">
            ...
            {job.highlights.map((highlight, i) => (
              <div key={i} className="space-y-3">
                <div className="flex items-start gap-3">
                  <span className="mt-1.5 flex-shrink-0 w-2 h-2 rounded-full bg-accent group-hover:scale-125 transition-transform duration-300 shadow-[0_0_8px_#d97706]"></span>
                ...
                {typeof highlight !== 'string' && highlight.subItems && (
                  <div className="ml-5 pl-4 border-l-2 border-slate-200 dark:border-[#2a2a2a] space-y-2.5 my-2">
