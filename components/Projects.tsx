...
export const Projects: React.FC = () => (
  <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
    {RESUME_DATA.projects.map((project, index) => {
      return <motion.article key={project.name} initial={{ opacity: 0, y: 22 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: '-50px' }} transition={{ duration: .45, delay: Math.min(index * .05, .25) }} className="group relative min-h-[320px] overflow-hidden rounded-[2rem] border border-slate-200 bg-[#fff8eb] p-6 transition-all hover:-translate-y-1 hover:border-accent/50 hover:shadow-[0_18px_50px_-26px_rgba(217,119,6,.55)] dark:border-[#303030] dark:bg-[#121212]">
        <div className="absolute inset-0 opacity-70 [background-image:radial-gradient(circle_at_1px_1px,rgba(217,119,6,.22)_1px,transparent_0)] [background-size:18px_18px] dark:opacity-30" />
